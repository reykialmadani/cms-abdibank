    ...
    const fetchVisitorData = async () => {
      try {
        ...
        const response = await axios.get<ApiResponse>("/api/getMonthlyVisitData", {
          params: { month: selectedMonth, year: selectedYear, daily: false },
        });
        ...
        if (response.data && Array.isArray(response.data.data)) {
          const sortedData = response.data.data
            ...
            .slice(0, 5)
            .map((item) => ({ name: item.subMenuName, value: item.visitCount }));
          setData(sortedData);
        } else {
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="mb-4 flex space-x-4">
        <div>
          ...
          <select id="monthSelect" value={selectedMonth} onChange={handleMonthChange} className="p-2 border rounded text-black">
        ...
        <div>
          ...
          <select id="yearSelect" value={selectedYear} onChange={handleYearChange} className="p-2 border rounded text-black">
            {years.map(year => (
      ...
      {loading ? (
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={data} cx="50%" cy="50%" outerRadius={100} fill="#8884d8" dataKey="value" label={({ name, value }) => `${name} (${value})`}>
                {data.map((_, index) => (
