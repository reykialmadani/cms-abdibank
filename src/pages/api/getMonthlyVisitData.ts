...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    const requestedMonth = req.query.month 
      ? parseInt(req.query.month as string, 10) - 1 
      : new Date().getMonth();
    ...
    const uniqueVisitors = await Promise.all(
      visits.map(async (visit) => {
        // Menambahkan type assertion untuk hasil query
        const uniqueCount = await prisma.$queryRaw<UniqueCountResult[]>`
        ...
        return {
          ...
          uniqueCount: Number(uniqueCount[0]?.count || 0)
        };
      })
    );
